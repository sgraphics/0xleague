...
*/
contract AuditorWorkflow  is WorkflowBase, Ownable{
	struct Auditor {
		uint id;
		uint64 status;
		string name;
...
/*
	...
	* `Signed NDA` = `NDA`
*/
	function createProfile(string calldata name,string calldata image,string calldata twitter,string calldata telegram,string calldata description) external returns (uint256) {
...
*/
	function addScore(uint256 id,uint newScore) external returns (uint256) {
...
/*
	...
	* `Description` (String)
*/
	function update(uint256 id,string calldata name,string calldata image,string calldata twitter,string calldata telegram,string calldata description) external returns (uint256) {
		Auditor memory item = getItem(id);
		_assertOrAssignAuditor(item);
		_assertStatus(item, 0);
		_setItemIdByAuditor(item, 0);
...
/*
	...
	Access is specifically restricted to the user with the address from the `Auditor` property. If `Auditor` property is not yet set then the method caller becomes the objects `Auditor`.
*/
	function disableProfile(uint256 id) external returns (uint256) {
		Auditor memory item = getItem(id);
		_assertOrAssignAuditor(item);
		_assertStatus(item, 0);
		_setItemIdByAuditor(item, 0);
...
/*
	...
	* `Signed NDA` = `NDA`
*/
	function rejoin(uint256 id) external returns (uint256) {
		Auditor memory item = getItem(id);
		_assertOrAssignAuditor(item);
